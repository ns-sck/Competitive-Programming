...
public:
  ...
  void push(int i, int nl, int nr) {
    if (lazy[i] != 0) {
      int mid = (nl + nr) / 2;
      nodes[lch].apply(lazy[i], nl, mid);
      nodes[rch].apply(lazy[i], mid + 1, nr);
      lazy[lch] += lazy[i];
      lazy[rch] += lazy[i];
    }
  }

  void update(int val, int ql, int qr, int i, int nl, int nr) {
    if (ql > nr || qr < nl) {
      return;
    }
    if (nl >= ql && nr <= qr) {
    ...
    push(i, nl, nr);
    int mid = (nl + nr) / 2;
    update(val, ql, qr, lch, nl, mid);
    update(val, ql, qr, rch, mid + 1, nr);
    nodes[i] = nodes[lch].merge(nodes[rch]);
  }
